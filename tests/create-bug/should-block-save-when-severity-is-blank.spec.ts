...
test.describe('Required-field validation', () => {
  test('should-block-save-when-severity-is-blank', async ({
    authenticatedBoard,
    boardPage,
    createBugModal,
    page,
    ...
    // 3. Clear the Severity selection
    await createBugModal.clearSeveritySelection();
    ...
    await expect(createBugModal.validationAlert).toContainText('Severity is required.');
  });
});
